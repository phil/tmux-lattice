#!/usr/bin/env bash
#
# layout_lib.sh — Shared tmux layout tree parsing, manipulation, and serialization.
#
...
#   NODE_PARENT[$id]   = parent node ID, or "" for root
#   NODE_W[$id], NODE_H[$id], NODE_X[$id], NODE_Y[$id] = geometry
# ---------------------------------------------------------------------------

declare -A NODE_TYPE NODE_CHILDREN NODE_PANEID NODE_PARENT NODE_W NODE_H NODE_X NODE_Y
NEXT_ID=0
...
# Accepts an optional parent ID as $1.
parse_node() {
    local id=$NEXT_ID
    NEXT_ID=$((NEXT_ID + 1))
...
# ---------------------------------------------------------------------------
# Parent map — post-parse traversal to populate NODE_PARENT
#
# Call after parse_node to enable upward tree navigation (used by push_right).
# build_parent_map <node_id> <parent_id_or_empty>
# ---------------------------------------------------------------------------
...
build_parent_map() {
    ...
    [[ "${NODE_TYPE[$id]}" == "leaf" ]] && return
    local child
    for child in ${NODE_CHILDREN[$id]}; do
        build_parent_map "$child" "$id"
    done
}
